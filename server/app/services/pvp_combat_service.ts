...
function getTypeEffectiveness(attackerType: PokemonType, defenderTypes: PokemonType[]): number {
  return defenderTypes.reduce((mult, defType) => mult * getEffectiveness(attackerType, defType), 1.0)
}
...
function computePokemonDps(
  ...
  const typeMult = attackerTypes.length > 0
...
export async function pickBoss(commonGenerations: number[]): Promise<Species | null> {
  ...
  const LEGENDARY_SLUGS = new Set([
    ...
    // Gen 5
    'cobalion', 'terrakion', 'virizion', 'tornadus', 'thundurus', 'landorus', 'reshiram', 'zekrom', 'kyurem', 'keldeo', 'meloetta', 'genesect', 'victini',
    // Gen 6
    'xerneas', 'yveltal', 'zygarde', 'diancie', 'hoopa', 'volcanion',
    // Gen 7
    'tapukoko', 'tapulele', 'tapubulu', 'tapufini', 'solgaleo', 'lunala', 'necrozma', 'magearna', 'marshadow', 'zeraora',
    // Gen 8
    'zacian', 'zamazenta', 'eternatus', 'calyrex', 'regieleki', 'regidrago', 'glastrier', 'spectrier',
    // Gen 9
  ...
  const EPIC_SLUGS = new Set([
    ...
    'charizard', 'blastoise', 'venusaur', 'gengar', 'alakazam', 'machamp',
    'arcanine', 'gyarados', 'lapras', 'snorlax',
    'typhlosion', 'feraligatr', 'meganium', 'scizor', 'heracross', 'kingdra',
    'blaziken', 'swampert', 'sceptile', 'gardevoir', 'aggron', 'milotic', 'absol',
    'infernape', 'empoleon', 'torterra', 'lucario', 'togekiss', 'garchomp',
    'volcarona', 'haxorus', 'chandelure', 'zoroark',
    'greninja', 'aegislash', 'talonflame', 'noivern',
    'decidueye', 'incineroar', 'primarina', 'golisopod', 'mimikyu',
    'cinderace', 'rillaboom', 'inteleon', 'corviknight', 'dragapult', 'grimmsnarl',
    'meowscarada', 'skeledirge', 'quaquaval', 'tinkaton', 'kingambit', 'gholdengo',
  ])
...
export async function resolveMatch(
  ...
  function buildSnapshot(pokemons: typeof p1Pokemon): { snapshot: PvpTeamSnapshot[]; totalDamage: number } {
    let totalDps = 0
